...
$data = $mysqli->query("SELECT * FROM managers WHERE name = '$name'");
$permission = mysqli_fetch_array($data, MYSQLI_ASSOC);
...
if($_POST['managerid'] && $_POST['pass'])
{
	...
	{
		if($_POST['post_edit_premission'] == ""){ $pep = 0; }
		if($_POST['post_delete_premission'] == ""){ $pdp = 0; }
		if($_POST['image_view_premission'] == ""){ $ivp = 0; }
		if($_POST['image_delete_premission'] == ""){ $idp = 0; }
		if($_POST['maintenance_view_premission'] == ""){ $mvp = 0; }
		if($_POST['maintenance_edit_premission'] == ""){ $mep = 0; }
		if($_POST['system_premission'] == ""){ $sp = 0; }

		$db = $mysqli->query("INSERT INTO managers(manager,pass,name,post_edit,post_delete,image_view,image_delete,maintenance_view,maintenance_edit,manager_system) VALUES('$createid','$passsave','DWE_$createid',$pep,$pdp,$ivp,$idp,$mvp,$mep,$sp)");
		if(!$db) { echo "保存失敗"; }
...
else if($_POST['comment_editid'])
{
	$_SESSION['scrolly'] = $_POST['pagey'];
	$comment_id = $_POST['comment_editid'];
	$comment_message = $_POST['comment_messages'];
	$comment_edit = $mysqli->query("UPDATE comments SET comment='$comment_message' WHERE id = $comment_id");

	$_POST['comment_editid'] = NULL;
	$_POST['comment_messages'] = NULL;
	header('Location: managers.php');
	exit();
}
else if($_POST['comment_deleteid'])
{
	$_SESSION['scrolly'] = $_POST['pagey'];
	$comment_id = $_POST['comment_deleteid'];

	$comment_delete = $mysqli->query("DELETE FROM comments WHERE id = $comment_id");

	$_POST['comment_deleteid'] = NULL;
	header('Location: managers.php');
	exit();
}
...
$comment = $mysqli->query("SELECT * FROM comments ORDER BY id DESC");
		...
		<div class="parent">
			<div class="columns">
				<form action="managers.php" method="post">
					...
					<button class="chooses" type="submit" name="function" value="cmt_edit">コメント編集</button>
					<button class="chooses" type="submit" name="function" value="cmt_delete">コメント削除</button>
			...
			<?php switch($function) { 
				...
				<?php if($permission['manager_system'] == TRUE) { ?>
				<form action="managers.php" method="post">
					<h4>管理者ID</h4>
					...
					<h3>権限</h3>
					<label><input class="check" type="checkbox" name="post_edit_premission" value="true">編集</label>　
					<label><input class="check" type="checkbox" name="post_delete_premission" value="true">削除</label>　
					<label><input class="check" type="checkbox" name="image_view_premission" value="true">画像一覧</label>　
					<label><input class="check" type="checkbox" name="image_delete_premission" value="true">画像削除</label>　
					<label><input class="check" type="checkbox" name="maintenance_view_premission" value="true">メンテナンス期間一覧</label>　
					<label><input class="check" type="checkbox" name="maintenance_edit_premission" value="true">メンテナンス設定</label>
					<br><br><label><input class="check" type="checkbox" name="system_premission" value="true">システム</label>

					<br><br><button class="button2" type="submit">作成</button>
					<br><p style="color: <?php echo $_SESSION['color']; ?>"><?php echo $_SESSION['msg']; ?></p>
				</form></center>
				<?php } else { ?><p align="center">アクセス権限無し</p><?php }
				break;
				...
				//============================
				case "edit": ?>
					<h2 align="center" style="color:lime">投稿内容 編集</h2>

				<?php if($permission['post_edit'] == TRUE) {
					foreach($data as $row): ?>
				<div class="comment">
				...
				<?php endforeach; } else { ?><p align="center">アクセス権限無し</p><?php }
				break;
				...
				//============================
				case "delete": ?>
					<h2 align="center" style="color:red">投稿内容 削除</h2>

				<?php if($permission['post_delete'] == TRUE) {
					foreach($data as $row): ?>
				<div class="comment">
					<p class="commentname">投稿ID：<?php echo $row['id']; ?></p>
					<?php if(!$row['accountid']) {?>
				...
				//============================
				case "images": ?>
				<h2 align="center">画像一覧</h2>

				<?php if($permission['image_view'] == TRUE) {
					foreach($data as $row): 
					if($row['image']) { ?>
					<div class="comment">
				...
				<?php }
				endforeach; } else { ?><p align="center">アクセス権限無し</p><?php }
				break;
				...
				//============================
				case "imagesdelete": ?>
				<h2 align="center">画像削除</h2>

				<?php if($permission['image_delete'] == TRUE) {
					foreach($data as $row): 
					if($row['image']) { ?>
					<div class="comment">
					<p class="commentname">画像ID：<?php echo $row['id']; ?></p>
				...
				//============================
				case "cmt_edit": ?>
					<h2 align="center" style="color:lime">コメント編集</h2>
					<?php if($permission['post_edit'] == TRUE) { 
					foreach($comment as $row):	?>
					<div class="comment">
						<p class="commentname">コメントID：<?php echo $row['id']; ?></p>
						<p class="commentname">投稿ID：<?php echo $row['post_id']; ?></p>
						<p class="commentname">名前 : <?php echo $row['name']; ?></p>
						<p class="commenttime">時刻 : <?php echo $row['commenttime']; ?></p>
						<p class="commentname">画像：<?php if($row['image']) { echo $row['image']; } else { echo "無し"; } ?></p>

						<form action="managers.php" method="post">
							<p class="info">コメント : </p><textarea class="textbox" name="comment_messages"><?php echo $row['comment']; ?></textarea>
							<input id="pageload" name="pagey" class="save" hidden>
							<div class="buttons"><button onclick="setScrollPositionsInHiddenFields()" type="submit" class="button2" name="comment_editid" value="<?php echo $row['id']; ?>">編集</button></div>
						</form>
					</div>
					<?php endforeach; } else { ?><p align="center">アクセス権限無し</p><?php }
				break;
				...
				//============================
				case "cmt_delete": ?>
					<h2 align="center" style="color:red">コメント削除</h2>
					<?php if($permission['post_delete'] == TRUE) {
					foreach($comment as $row): ?>
					<div class="comment">
					<p class="commentname">コメントID：<?php echo $row['id']; ?></p>
					...
					<p class="info">投稿内容 : </p><textarea class="textbox" readonly><?php echo $row['comment']; ?></textarea>
					<form action="managers.php" method="post">
						<input id="pageload" name="pagey" class="save" hidden>
						<div class="buttons"><button onclick="setScrollPositionsInHiddenFields()" type="submit" class="button2" name="comment_deleteid" value="<?php echo $row['id']; ?>">削除</button></div>
					</form>
					</div>
					<?php endforeach; } else { ?><p align="center">アクセス権限無し</p><?php }
				break;
				...
				case "maintenance": ?>
					...
					<?php if($permission['maintenance_edit'] == TRUE) { ?>
					<form action="managers.php" method="post">
				...
				case "showmaintenance": ?>
					...
					<?php if($permission['maintenance_view'] == TRUE) { ?>
					<h3>実行中</h3>
